...
bool CompileState::compile(bool allowCpuFallback) {
  ...
  auto getOperatorProperties =
      [&registry, this, &isValidPlanNodeId, ctx](const exec::Operator* op) {
        ...
        if (isAnyOf<CudfBatchConcat>(op)) {
          // Inserted outside the adapter loop, so it IS re-examined here and
          // no adapter matches it. CudfOperatorBase does NOT inherit
          // CudfOperator, so the isAnyOf<CudfOperator> test above does not
          // catch it; without this it is judged a CPU operator and a
          // CudfToVelox is inserted in front, tripping its "expects CudfVector
          // input" check. It consumes and produces GPU batches.
          props.canRunOnGPU = true;
          props.acceptsGpuInput = true;
          props.producesGpuOutput = true;
        }
  ...
  for (int32_t operatorIndex = 0; operatorIndex < operators.size();
    ...
    if (adapter) {
      ...
      //
      // CudfBatchConcat has no adapter and does not inherit CudfOperator;
      // omitting it makes the concat a "pure CPU" operator, which aborts the
      // task outright when allowCpuFallback=false. See the matching isAnyOf<>
      // in the properties lambda above -- these two lists have to agree.
      if (isAnyOf<CudfOperator>(oper) ||
          isAnyOf<CudfBatchConcat>(oper)) {
        isPureCpuOperator = false;
      } else {
