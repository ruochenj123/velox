...
TpchPlan TpchQueryBuilder::buildQueryPlan(int queryId) const {
  switch (queryId) {
    ...
    case 32:
      return getCaseAPlan();
    case 44:
      return getSortScanPlan();
    case 45:
      return getSortJoinPlan();
...
// Q44 (row-sort microbench, branch row-sort 2026-08-24): TableScan -> gather
// -> ORDER BY, no join, no aggregation; the sorted rows go straight to the
// CPU. lineitem filtered to l_shipdate < '1993-01-01' (~14% of rows) so the
// sort fits GPU memory at SF100 (~86M rows). Keys first so the 5-column
// result printer shows them; the payload (fixed + 2 VARCHARs) is what the
// row gather / deferral carries.
// SELECT l_orderkey, l_linenumber, l_partkey, l_suppkey, l_quantity,
//        l_extendedprice, l_discount, l_shipdate, l_returnflag, l_shipmode
// FROM lineitem WHERE l_shipdate < DATE '1993-01-01'
// ORDER BY l_orderkey, l_linenumber
TpchPlan TpchQueryBuilder::getSortScanPlan() const {
  std::vector<std::string> lineitemColumns = {
      "l_orderkey",
      "l_linenumber",
      "l_partkey",
  ...
  const auto& lineitemFileColumns = getFileColumnNames(kLineitem);
  auto shipDateFilter = formatDateFilter(
      "l_shipdate", lineitemSelectedRowType, "", "'1992-12-31'");
  ...
  auto plan = PlanBuilder(planNodeIdGenerator, pool_.get())
                  ...
                  .captureScanNodeId(lineitemPlanNodeId)
                  .localPartition(std::vector<std::string>{})
                  .orderBy({"l_orderkey", "l_linenumber"}, false)
                  .planNode();
...
// Q45 (row-sort microbench): JOIN -> gather -> ORDER BY -> CPU. lineitem
...
TpchPlan TpchQueryBuilder::getSortJoinPlan() const {
  ...
  auto orderDateFilter = formatDateFilter(
      "o_orderdate", ordersSelectedRowType, "'1998-01-01'", "");
